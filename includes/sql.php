...
// insert customer
function create_customers($fname, $lname, $credit_limit, $balance_status) {
  global $db;
  $fname = $db->escape($fname);
  $lname = $db->escape($lname);
  $credit_limit = $db->escape($credit_limit);
  $balance_status = $db->escape($balance_status);
  $sql = "INSERT INTO customers (fname, lname, credit_limit, balance_status) VALUES ('{$fname}','{$lname}','{$credit_limit}','{$balance_status}')";
  return $db->query($sql);
}
// update customer
function update_customers($id, $fname, $lname, $credit_limit, $balance_status) {
  global $db;
  $id = $db->escape($id);
  $fname = $db->escape($fname);
  $lname = $db->escape($lname);
  $credit_limit = $db->escape($credit_limit);
  $balance_status = $db->escape($balance_status);
  $sql = "UPDATE customers SET fname = '{$fname}', lname = '{$lname}', credit_limit = '{$credit_limit}', balance_status = '{$balance_status}') WHERE id = '{$id}'";
  return $db->query($sql);
}
...
function get_transaction_by_id($id) {
  global $db;
  $id = $db->escape($id);
  $result = find_by_id('transactions', $id);
  return $result;
}
